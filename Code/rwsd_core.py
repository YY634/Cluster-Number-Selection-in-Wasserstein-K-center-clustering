...
Core algorithms for RWSD experiments: WSD depth, Sinkhorn distances,
free-support Wasserstein barycenter / IRLS median, K-means and K-median
clustering on empirical point clouds, validation over K with silhouette /
ReD / DBI.

This module is a faithful port of the setup cell of
``rwsd_simulations_data_generators_in_experiments.ipynb``. Data generation
functions live in ``rwsd_experiments.py``.
"""
...
def list_to_mnd2_equal(clouds):
    ...
    used by WSD routines, assuming equal weights within each cloud.
    """
...
def _sinkhorn_divergence_log(X, Y, reg=0.12):
    """Sinkhorn divergence with uniform weights (log-domain)."""
...
def compute_pairwise_D(distributions, reg=0.12, n_jobs=-1, normalize_global_std=False, verbose=True):
    """Pairwise Sinkhorn distances between empirical clouds."""
    dists = distributions
...
def sinkhorn_log_domain(C, a, b, eps, niter=600, f0=None, g0=None, tol=1e-9):
    """Log-domain Sinkhorn iterations with optional warm-start."""
    k, m = C.shape
...
def free_support_barycenter(clouds, k, eps_abs, scale2,
                            ...
                            seed=0):
    """Semi-free-support entropic Wasserstein barycenter."""
...
# ===================== Free-support K-median clustering =====================
...
def wasserstein_k_median_clustering(
    ...
    for r in range(max_outer_rounds):
        ...
        for i, row in results:
            D_assign[i] = row
        new_labels = np.argmin(D_assign + 1e-12*rng.standard_normal(D_assign.shape), axis=1)
    ...
    # Ensure centers match final labels (refit if loop exited by max_outer_rounds
    # with non-zero changes on the last round).
    if r == max_outer_rounds - 1 and changes != 0:
        clusters = [np.where(labels==k)[0] for k in range(K)]
        def _fit_final(k):
            idx = clusters[k]
            if len(idx) == 0:
            ...
            return dict(support=Xk, weights=ak, size=len(idx), members=idx)
        centers = Parallel(n_jobs=n_jobs, backend="loky")(delayed(_fit_final)(k) for k in range(K))

    return labels, centers

# ===================== DBI from free-support medians =====================

def dbi_centroid_like_from_centers(distributions, labels, centers, reg=0.12):
    """
    Davies-Bouldin Index using free-support K-median prototypes.
      S_i  = mean Wasserstein distance from members to cluster prototype.
      M_ij = Wasserstein distance between prototypes i and j.
    """
    ...
    # Prototypes keyed by the actual cluster label.
    proto = {}
    for c in cats:
        info = centers[c] if centers[c] is not None else None
        if info is None:
            return np.nan
    ...
    # Between-prototype distances.
    cats_list = list(cats)
    M = np.zeros((K, K), dtype=float)
    for a_idx, ca in enumerate(cats_list):
        Xa, aa = proto[ca]
        for b_idx, cb in enumerate(cats_list):
            if a_idx == b_idx:
                continue
            Xb, ab = proto[cb]
    ...
    # DBI.
    Rmax = []
    for i_idx, ci in enumerate(cats_list):
        Si = S[ci]
        worst = -np.inf
        for j_idx, cj in enumerate(cats_list):
            if i_idx == j_idx:
                continue
            Sj = S[cj]
            Mij = M[i_idx, j_idx]
...
def evaluate_with_labels(
    ...
    """
    Evaluate a clustering: pairwise-distance matrix, silhouette, mean ReD, DBI.
    """
...
def validate_over_Ks_kmedian_free(distributions, Ks=(2,3,4,5),
    ...
    Metrics per K: silhouette, ReD (WSD-based), DBI (via free-support medians).
    """
    ...
    clouds = np.stack(distributions, axis=0)
    ...
    # Filter NaN/inf independently for each metric.
    Ks_ok_sil = [k for k in Ks if np.isfinite(sil_vals[k])]
    Ks_ok_red = [k for k in Ks if np.isfinite(red_vals[k])]
    Ks_ok_dbi = [k for k in Ks if np.isfinite(dbi_vals[k])]
    K_star_sil = max(Ks_ok_sil, key=lambda k: sil_vals[k]) if Ks_ok_sil else None
    K_star_red = max(Ks_ok_red, key=lambda k: red_vals[k]) if Ks_ok_red else None
    K_star_dbi = min(Ks_ok_dbi, key=lambda k: dbi_vals[k]) if Ks_ok_dbi else None
...
def plot_clouds_by_label(distributions, y_true, centers=None, title=None, save_path=None, xlim=None, ylim=None):
    """Generic 2D empirical cloud plot."""
    colors = {0: "#1f77b4", 1: "lightpink", 2: "#ff7f0e", -1: "#7f7f7f"}
    plt.figure(figsize=(6, 6))
    for i, X in enumerate(distributions):
        plt.scatter(X[:, 0], X[:, 1], s=6, c=colors.get(int(y_true[i]), "#7f7f7f"), alpha=0.25)
    if centers is not None:
        for c in centers:
            plt.scatter(c[0], c[1], c="k", s=160, marker="*", edgecolors="w", linewidths=1.0)
    handles = []
    for lab in sorted(set(int(x) for x in np.unique(y_true))):
        label = "Noise" if lab == -1 else f"Cluster {lab + 1}"
        handles.append(Line2D([0], [0], marker="o", color="w", label=label, markerfacecolor=colors.get(lab, "#7f7f7f"), markersize=8, alpha=0.8))
...
def print_best_summary(out):
    """Print selected K under all three validation criteria."""
    print("\nBest by Silhouette:", out.get("K_star_sil"))
    print("Best by ReD:", out.get("K_star_red"))
    print("Best by DBI:", out.get("K_star_dbi"))


def default_clustering_kwargs(seed=42, n_jobs=-1):
    """Default clustering parameters used in the supplied simulation scripts."""
    return dict(
        k_support=50,
        eps_abs=1e-3,
        outer_it=35,
...
# ============================================================
# Free-support Wasserstein K-means clustering and validation
# ============================================================

def wasserstein_k_means_clustering(
    clouds, K, k_support,
    eps_abs=1e-3, outer_it=35, inner_it=600,
    seed=42, n_jobs=-1, max_outer_rounds=8,
    pam_warm_start=False, D_for_pam=None
):
    """
    Free-support Wasserstein K-means clustering of empirical point clouds.

    The center update uses free_support_barycenter. This function is the K-means
    counterpart of wasserstein_k_median_clustering and preserves the same
    assignment and validation interface.
    """
    rng = np.random.default_rng(seed)
    n, m, d = clouds.shape
    scale2 = _median_scale2(clouds, rng=rng)
    ...
    for r in range(max_outer_rounds):
        ...
        def _fit_one(k):
            ...
            sub = clouds[idx]
            Xk, ak, fg, Klogs = free_support_barycenter(
                sub,
                k=k_support,
                eps_abs=eps_abs,
                scale2=scale2,
        ...
        centers = Parallel(n_jobs=n_jobs, backend="loky")(
            delayed(_fit_one)(k) for k in range(K)
        )
        ...
        def _cost_row(i):
            ...
            for k in range(K):
                if centers[k] is None:
                    ...
                    Xk = centers[k]["support"]
                    ak = centers[k]["weights"]
                    row[k] = ot_epsilon_value(
                        Xk, ak, clouds[i],
                        eps_abs=eps_abs,
                        scale2=scale2,
                        inner_it=inner_it // 2
        ...
        results = Parallel(n_jobs=n_jobs, backend="loky")(
            delayed(_cost_row)(i) for i in range(n)
        )
        ...
        new_labels = np.argmin(
            D_assign + 1e-12 * rng.standard_normal(D_assign.shape),
            axis=1
        )

        changes = int(np.sum(new_labels != labels))
        print(f"[WS-kmeans round {r + 1}] reassignment changes: {changes}")

        labels = new_labels
        if changes == 0:
            break

    if max_outer_rounds > 0 and r == max_outer_rounds - 1 and changes not in (None, 0):
        clusters = [np.where(labels == k)[0] for k in range(K)]
        ...
        centers = Parallel(n_jobs=n_jobs, backend="loky")(
            delayed(_fit_final)(k) for k in range(K)
        )

    return labels, centers


def validate_over_Ks_kmeans_free(
    distributions, Ks=(2, 3, 4, 5),
    clustering_kwargs=None,
    reg=0.12,
    ...
    """
    Validate K for free-support Wasserstein K-means clustering.

    Metrics per K:
      Silhouette
      ReD or RWSD mean
      DBI using free-support Wasserstein barycenter prototypes
    """
    if clustering_kwargs is None:
    ...
    for K in Ks:
        print(f"\n=== Free-support WS K-means with K={K} ===")

        labs, centers = wasserstein_k_means_clustering(
            clouds,
            K,
            pam_warm_start=use_pam_warm_start,
            D_for_pam=D_all,
            **clustering_kwargs
        )
    ...
    print("\n=== Summary over K (free-support WS K-means) ===")
    for K in Ks:
        print(
            f"K={K}: Silhouette={sil_vals[K]:.4f} | "
            f"ReD(mean)={red_vals[K]:.4f} | DBI={dbi_vals[K]:.4f}"
        )
    ...
    if plot:
        ...
        plt.plot(Ks_sorted, [sil_vals[k] for k in Ks_sorted], marker="o")
        if K_star_sil is not None:
            plt.axvline(K_star_sil, ls="--", alpha=0.6)
        ...
        plt.plot(Ks_sorted, [red_vals[k] for k in Ks_sorted], marker="o")
        if K_star_red is not None:
            plt.axvline(K_star_red, ls="--", alpha=0.6)
        plt.title("ReD (WSD) vs K")
        plt.xlabel("K")
        plt.ylabel("ReD mean")
        plt.tight_layout()
        plt.show()
        ...
        plt.plot(Ks_sorted, [dbi_vals[k] for k in Ks_sorted], marker="o")
        if K_star_dbi is not None:
            plt.axvline(K_star_dbi, ls="--", alpha=0.6)
        plt.title("DBI vs K")
        plt.xlabel("K")
    ...
    return dict(
        ...
        labels_perK=labels_perK,
        centers_perK=centers_perK
    )


# ============================================================
# Notebook configuration and validation dispatch
# ============================================================
...
def validate_by_method(
    ...
    """
    Dispatch to the requested Wasserstein clustering validation routine.

    method='kmeans' uses validate_over_Ks_kmeans_free.
    method='kmedian' uses validate_over_Ks_kmedian_free.
    """
    clustering_kwargs = default_clustering_kwargs(seed=config.seed, n_jobs=config.n_jobs)

    if method == "kmeans":
        return validate_over_Ks_kmeans_free(
            distributions,
            Ks=tuple(Ks),
            ...
            use_pam_warm_start=use_pam_warm_start,
        )

    if method == "kmedian":
        return validate_over_Ks_kmedian_free(
            distributions,
            Ks=tuple(Ks),
            clustering_kwargs=clustering_kwargs,
            reg=config.reg,
    ...
    raise ValueError("method must be either 'kmeans' or 'kmedian'.")
